...
type MutexValueReceiver struct {
	mutex sync.Mutex
	data  int
}

func (m MutexValueReceiver) Increment() {
	m.mutex.Lock()
	defer m.mutex.Unlock()
	fmt.Println("值接受者MutexValueReceiver.Increment方法中data地址:", &m.data)
	m.data++
}

func (m MutexValueReceiver) GetValue() int {
	m.mutex.Lock()
	defer m.mutex.Unlock()
	fmt.Println("值接受者MutexValueReceiver.GetValue方法中data地址:", &m.data)
	return m.data
}

func main() {
	s := MutexValueReceiver{data: 42}
	fmt.Println("初始化MutexValueReceiver后data的指针值：", &s.data)
	s.Increment()
...
/*
输出结果:
初始化MutexValueReceiver后data的指针值： 0x1400009c028
值接受者MutexValueReceiver.Increment方法中data地址: 0x1400009c038
值接受者MutexValueReceiver.GetValue方法中data地址: 0x1400009c048
42
*/

// 可见引用类型使用单纯的值接受者,是值接收器是将方法与结构体的值关联,复制结构体字段的值
